...
impl Sheet {
	fn get_next_value(file_path: &str, line_no: usize, line: &str, start: usize) -> Option<Cell>{
			...
			match curr {
				...
				Some(ref mut v) => match v {
					...
					CellValue::Number(ref mut n) => {if chr == ',' || chr == ')' {
						*n = line[start..i].trim().replace("_", "").parse().unwrap();
						return Some(Cell {
							value: v.clone(),
							string_backing: Some(line[start..i].to_string()),
							changed: false,
					...
					CellValue::Formula(ref mut f) => todo!(),
					_ => unreachable!(),
				}
			}
			i+=1;
		...
		if let Some(ref mut v) = curr {
			match v {
				CellValue::String(_) => {},
				CellValue::Number(ref mut n) => { *n = line[start..].trim().replace("_", "").parse().unwrap(); }
	...
	pub fn new(file_path:String, str: String) -> Self {
		return Sheet { cells: str.replace("\r","").split("\n").enumerate().map(|(i, line)| {
			...
			let mut start = 0;
			let mut cell = Self::get_next_value(&file_path, i, line, start);
			while let Some(ref c) = cell {
				rv.push((*c).clone());
				start += c.string_backing.as_ref().unwrap().len() + 1;
				cell = Self::get_next_value(&file_path, i, line, start);
			}
			return rv;
